...
from .models import Profile, Hashtag
...
def create_post_view(request):
    if request.method == 'POST':
        ...
        if post_form.is_valid():
            post_inst = post_form.save()
            new_hashtags = post_form.cleaned_data.get('new_hashtags')
            hashtags_list = new_hashtags.split()
            for hashtag in hashtags_list:
                Hashtag.objects.get_or_create(name=hashtag)
                hashtag_objs = Hashtag.objects.filter(name__in=hashtags_list)
                post_inst.hashtags.add(*hashtag_objs)
            return redirect('profile-details', profile_id=post_inst.author.id)
        else:
